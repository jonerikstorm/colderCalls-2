...
// Open a CSV file from Google Docs
/*
Do all the Google Authorization shit for drive, sheets, classroom, and gmail
...
HERE WE SHOULD OPEN A FILE PICKER OR CREATE NEW
  ...
  return "no id"; // eventually
    ...
    var spreadsheetId = '';
    var globalPrefs:Dict = Sheets.Spreadsheets.Values.get(spreadsheetId,'Global Prefs!A2:A10)';
    var periodPrefs:Dict = Sheets.Spreadsheets.Values.get(spreadsheetId, 'Period Prefs!A2:A'+globalPrefs.numPeriods);
    var students:Dict = Sheets.Spreadsheets.Values.get(spreadsheetId, 'Students!A2:A'+highestNumberWithEntry);
...
//    updatePrefs();
    ...
    let gPT = $('globalPrefsTab');
    gPT.on('click', function (e) {
        e.preventDefault();
    ...
    var dontPick: Array<number>;
    $("#selectedStudentJumbotron").html(selectStudent(students, dontPick));
...
async function writeStudent(index)
{
...
{
    let value:string = String($('#biasSlide'+index).val());
...
//
//
async function writeGlobalPrefs() {
    const statusBarText = $("#statusBar").html();
    $("#statusBar").html(statusBarText+'<div class="spinner-border spinner-border-sm"></div>');
   /*
    $.post("random.php",
...
function spin(selector:string): Boolean {
    $(selector).html($(selector).html() + '<div class="spinner-border spinner-border-sm"></div>');
    return true;
}

async function writePeriodPrefs(period) {
    const statusBarText = $("#statusBar").html();
    spin("#statusBar");
    await gapi.document.write("")
    /*
    $.post("random.php",
        {
...
{
    let value:string = String($('#betweenSlide'+period).val());
